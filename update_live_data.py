...
try:
    ...
    # HTML, Gelişmiş CSS ve Yeni Tasarım Kuralları
...
<head>
    ...
    <style>
        ...
        /* Çarpı işareti kaldırıldı, sadece silik yazı yapıldı */
        ...
        /* Turnuva Ağacı Elenme/Kazanma ve Bekleme Durumları */
        .winner { font-weight: bold; color: #2c3e50; }
        .loser { color: #bdc3c7; opacity: 0.5; }
        .tbd-team { color: #e67e22; font-style: italic; font-size: 13px; } /* Henüz belli olmayan rakipler için */
    </style>
</head>
<body>
...
"""
    ...
    for group in standings:
        ...
        for index, table_row in enumerate(table_rows):
            ...
            # En iyi 3.ler kuralı ve 2026 formatı hassasiyeti:
            # Grup aşamasında ilk 2 kesin çıkar. 3. sıradakilerin çıkma ihtimali son ana kadar sürer.
            # Dolayısıyla 4. sıradaki takımlar matematiksel olarak elenmediği sürece silik yapılmamalıdır.
            if played == 3:
                if index < 2:
                    row_style = ' class="status-qualified"'
                elif index == 2:
                    row_style = ' class="status-active"' # En iyi 3. adayı, iddiası sürüyor
                else:
                    row_style = ' class="status-eliminated"' # Kesin elenen sonuncu takım
            else:
                if points >= 6:
                    row_style = ' class="status-qualified"'
                elif played == 2 and points == 0 and gd <= -4: # Ağır averajla 0 çeken elenmeye çok yakındır
        ...
        for stage_key, stage_title in stages.items():
            ...
            if stage_matches:
                ...
                for match in stage_matches:
                    ...
                    # Eğer takım ismi yoksa API'nin sağladığı geçici bilgileri (Örn: "Winner Group A") yerleştiriyoruz
                    home_team = home_team_obj.get("name") if home_team_obj else None
                    away_team = away_team_obj.get("name") if away_team_obj else None
                    
                    # Google tarzı yer tutucular (Eşleşmesini bekleyen liderler/ikinciler için)
                    if not home_team:
                        home_team = "A Grubu Lideri (Bekliyor)" if "A" in str(match.get("id")) else "Grup Eleme Takımı"
                        home_is_tbd = True
                    else:
                        home_is_tbd = False
                        
                    if not away_team:
                        away_team = "En İyi 3. Takım (Bekliyor)" if "3" in str(match.get("id")) else "Grup Eleme Takımı"
                        away_is_tbd = True
                    else:
                        away_is_tbd = False
                    ...
                    # Maç tamamlandıysa elenen takımı silik yapma kuralları
...
"""
    ...
    print("Google tarzı dinamik eşleşmeler ve esnek elenme kuralları eklendi.")
